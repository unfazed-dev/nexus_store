...
/// Base sealed class for store state management with signals.
///
/// This class represents all possible states for a list-based store operation:
/// - [NexusSignalInitial]: Before first load
/// - [NexusSignalLoading]: Loading (with optional previous data)
/// - [NexusSignalData]: Success with data
/// - [NexusSignalError]: Error (with optional previous data)
sealed class NexusSignalState<T> {
  const NexusSignalState();
...
final class NexusSignalInitial<T> extends NexusSignalState<T> {
  ...
  bool operator ==(Object other) =>
      ...
      other is NexusSignalInitial<T> && runtimeType == other.runtimeType;

  @override
  int get hashCode => runtimeType.hashCode;

  @override
  String toString() => 'NexusSignalInitial<$T>()';
}

/// Loading state with optional previous data for optimistic UI.
final class NexusSignalLoading<T> extends NexusSignalState<T> {
  const NexusSignalLoading({this.previousData});
...
final class NexusSignalData<T> extends NexusSignalState<T> {
  const NexusSignalData({required this.data});
  ...
  @override
  String toString() => 'NexusSignalData<$T>(data: $data)';
}

/// Error state with optional previous data for recovery UI.
final class NexusSignalError<T> extends NexusSignalState<T> {
  NexusSignalError({
    required this.error,
    this.stackTrace,
    this.previousData,
  ...
  @override
  String toString() =>
      'NexusSignalError<$T>(error: $error, stackTrace: $stackTrace, previousData: $previousData)';
}
