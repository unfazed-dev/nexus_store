...
void main() {
  group('PowerSyncQueryTranslator.toAggregateSql', () {
    ...
    test('generates SUM without query', () {
      ...
      expect(sql, equals('SELECT SUM(amount) AS result FROM orders'));
      expect(args, isEmpty);
    });
    ...
    test('generates AVG without query', () {
      ...
      expect(sql, equals('SELECT AVG(rating) AS result FROM reviews'));
      expect(args, isEmpty);
    });
    ...
    test('generates MIN without query', () {
      ...
      expect(sql, equals('SELECT MIN(price) AS result FROM products'));
      expect(args, isEmpty);
    });
    ...
    test('generates MAX without query', () {
      ...
      expect(sql, equals('SELECT MAX(price) AS result FROM products'));
      expect(args, isEmpty);
    });
    ...
    test('generates AVG with multiple filters', () {
      ...
      expect(
        sql,
        equals('SELECT AVG(price) AS result FROM subscriptions '
    ...
    test('applies field mapping', () {
      final mapped = PowerSyncQueryTranslator<Map<String, dynamic>>(
      ...
      expect(sql, equals('SELECT SUM(total_amount) AS result FROM orders'));
      expect(args, isEmpty);
    });

    test('generates with null query (same as no query)', () {
      final (sql, args) = translator.toAggregateSql(
        tableName: 'orders',
        field: 'amount',
        type: AggregateType.sum,
