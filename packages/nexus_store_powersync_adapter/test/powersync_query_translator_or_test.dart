...
void main() {
  ...
  group('PowerSyncQueryTranslator OR logic', () {
    group('toSelectSql', () {
      ...
      test('handles single-condition OR group', () {
        final query = const Query<Map<String, dynamic>>()
            .where('status', isEqualTo: 'active')
            .or(
        ...
        expect(
          sql,
          equals('SELECT * FROM users WHERE status = ? AND (role = ?)'),
        );
        expect(args, equals(['active', 'admin']));
      });
    });
    ...
    group('fieldMapping with OR', () {
      test('applies field mapping to OR group conditions', () {
        final mappedTranslator = PowerSyncQueryTranslator<Map<String, dynamic>>(
